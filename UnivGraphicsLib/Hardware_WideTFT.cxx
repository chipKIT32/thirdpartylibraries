...
static void Wide_LCD_Write_Data(unsigned char data);
...
static void Trans_Dat(unsigned int index, unsigned int data)
{
	...
	Wide_LCD_Write_Data(high8);

	SET_LOW(kWideTFT_WR);

	SET_HIGH(kWideTFT_WR);
	Wide_LCD_Write_Data(low8);
	SET_LOW(kWideTFT_WR);
...
{
	Wide_LCD_Clear(0);	//	clear to black
}
...
//	Converts true color into 5:6:5 RGB format.
	...
	Wide_LCD_Pixel(xLoc, yLoc, the16bitColor);
}
...
{
	Wide_LCD_Pixel(xLoc, yLoc, theColorByte);
}
