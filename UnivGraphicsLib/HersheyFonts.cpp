...
{
//	strcpy_P(fontDefString, (char*)READ_PTR_FROM_PGM(&(gHershyFontTable[tableIndex])));
	GetFontDefinitionString(fontDefString, gHershyFontTable, tableIndex);
}
...
//*******************************************************************************
	...
	{
	//	strcpy_P(fontDefString, (char*)READ_PTR_FROM_PGM(&(gHershyCursiveFontTable[tableIndex])));
		GetFontDefinitionString(fontDefString, gHershyCursiveFontTable, tableIndex);
	}
#endif
	...
	{
	//	strcpy_P(fontDefString, (char*)READ_PTR_FROM_PGM(&(gHershyFuturalFontTable[tableIndex])));
		GetFontDefinitionString(fontDefString, gHershyFuturalFontTable, tableIndex);
	}
#endif
	...
	{
	//	strcpy_P(fontDefString, (char*)READ_PTR_FROM_PGM(&(gHershyFuturamFontTable[tableIndex])));
		GetFontDefinitionString(fontDefString, gHershyFuturamFontTable, tableIndex);
	}
#endif
	...
	{
	//	strcpy_P(fontDefString, (char*)READ_PTR_FROM_PGM(&(gHershyJapaneseFontTable[tableIndex])));
		GetFontDefinitionString(fontDefString, gHershyJapaneseFontTable, tableIndex);
	}
#endif
...
static void	GetHersheyFontStringFromIndex(char fontNumber, char theCharIndex, char *fontVectorString)
{
	...
	//********************************************************************************
		...
		case kHersheyFont_Cyrillic1:
			GetFontDef_Cyrillic1(theCharIndex, fontVectorString);
			break;
	#endif
	...
	while (theVectorString[ii] != 0)
	{
		...
		//	float y = -h2float(theVectorString[ii+1] );
			...
			double	doubleXX, doubleYY;
				...
				//*	first calculate the polar coordinates
				polarRadius			=	lround(sqrt((doubleXX * doubleXX) + (doubleYY * doubleYY)));
				polarTheta			=	atan(doubleYY / doubleXX);
				...
				polarTheta			-=	(doubleRotateAngle * DEG_TO_RAD);
				
				
				if (polarTheta > TWO_PI)
				{
					polarTheta	-=	TWO_PI;
				}
				
				//*	convert back to x,y
				newX		=	lround((polarRadius * cos(polarTheta)) / kMultiplyFactor);
				newY		=	lround((polarRadius * sin(polarTheta)) / kMultiplyFactor);
